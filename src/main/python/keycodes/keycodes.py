...
def create_custom_user_keycodes(custom_keycodes):
    ...
    for x, c_keycode in enumerate(custom_keycodes):
        render_label = c_keycode.get("shortName", "USER{:02}".format(x))
        if len(render_label) > 9:
            render_label = render_label.replace("_", " ")
            _original = render_label
            render_label = []
            # we gonna split into new lines - but max 3!
            lines_candidiates = _original.split(' ')
            _current_len = 0
            for chunk in lines_candidiates:
                _current_len += len(chunk)
                if _current_len > MAX_LINE_WIDTH:
                    render_label.append("\n" + chunk)
                    _current_len = 0
                    continue
                else:
                    if len(render_label) == 0:
                        render_label.append(chunk)
                        continue
                    elif render_label[-1][-1] == "\n":
                        render_label.append(chunk)
                        continue
                    else:
                        render_label.append(" " + chunk)
            ...
            render_label = "".join(render_label)
        KEYCODES_USER.append(
            Keycode(
                "USER{:02}".format(x),
                render_label,
